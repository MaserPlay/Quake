...
void LeafFlow( int leafnum )
{
  ...
  for ( i = 0; i < leaf->numportals; i++ ) {
    ...
    for ( j = 0; j < bitbytes; j++ ) outbuffer[j] |= p->visbits[j];
  }
...
void CalcVis( void )
{
  ...
  //
  for ( i = 0; i < portalleafs; i++ ) LeafFlow( i );
  ...
  for ( i = 0; i < source->numpoints; i++ ) {
    ...
    for ( j = 0; j < pass->numpoints; j++ ) {
      ...
      for ( k = 0; k < source->numpoints; k++ ) {
        ...
        if ( d <
             -ON_EPSILON ) {  // source is on the negative side, so we want all
          // pass and target on the positive side
          fliptest = false;
          break;
        } else if (
            d >
            ON_EPSILON ) {  // source is on the positive side, so we want all
...
int main( int argc, char **argv )
{
  ...
  printf( "J.A.C.K. modifications by Crystice Softworks\n" );
  printf( "------------ vis ------------\n" );
