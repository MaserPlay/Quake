...
#define NUM_TJUNC_HASH 1024

wedge_t *wedge_hash[NUM_TJUNC_HASH];

static vec3_t hash_min, hash_scale;

static void InitTJuncHash( vec3_t mins, vec3_t maxs )
{
  vec3_t size;
  vec_t volume;
  vec_t scale;
  ...
  scale = sqrt( volume / NUM_TJUNC_HASH );
...
static unsigned HashTJuncVec( vec3_t vec )
{
  unsigned h;
...
void SplitFaceForTjunc( face_t *f, face_t *original )
{
  ...
  do {
    if ( f->numpoints <=
         MAXPOINTS ) {  // the face is now small enough without more cutting
...
void tjunc_find_r( node_t *node )
{
  ...
  for ( f = node->faces; f; f = f->next ) AddFaceEdges( f );
