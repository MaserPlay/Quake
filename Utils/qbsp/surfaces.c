...
#define NUM_SURF_HASH 4096

hashvert_t *hashverts[NUM_SURF_HASH];

static vec3_t hash_min, hash_scale;

static void InitSurfHash( void )
{
  vec3_t size;
  vec_t volume;
  vec_t scale;
  ...
  scale = sqrt( volume / NUM_SURF_HASH );
...
static unsigned HashSurfVec( vec3_t vec )
{
  unsigned h;
...
void FindFaceEdges( face_t *face )
{
  ...
  for ( i = 0; i < face->numpoints; i++ )
    face->edges[i] =
        GetEdge( face->pts[i], face->pts[( i + 1 ) % face->numpoints], face );
}
  ...
  for ( i = 1; i < numedges; i++ ) {
    ...
    if ( !edgefaces[i][1] ) {
      ...
      qprintf(
          ...
          d1->point[0], d1->point[1], d1->point[2], d2->point[0], d2->point[1],
          d2->point[2] );
    } else {
      f1 = edgefaces[i][0];
      f2 = edgefaces[i][1];
...
void MakeFaceEdges_r( node_t *node )
{
  ...
  for ( f = node->faces; f; f = f->next ) FindFaceEdges( f );
