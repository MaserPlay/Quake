...
void MarkLeakTrail( portal_t *n2 )
{
  ...
  for ( i = 1; i < n2->winding->numpoints; i++ ) {
    for ( j = 0; j < 3; j++ ) p1[j] = p1[j] + n2->winding->points[i][j];
  }
  for ( j = 0; j < 3; j++ ) p1[j] /= n2->winding->numpoints;

  VectorCopy( n1->winding->points[0], p2 );
  for ( i = 1; i < n1->winding->numpoints; i++ ) {
    for ( j = 0; j < 3; j++ ) p2[j] = p2[j] + n1->winding->points[i][j];
  }
  for ( j = 0; j < 3; j++ ) p2[j] /= n1->winding->numpoints;
...
qboolean RecursiveFillOutside( node_t *l, qboolean fill )
{
  ...
  for ( p = l->portals; p; ) {
    ...
    if ( RecursiveFillOutside( p->nodes[s],
                               fill ) ) {  // leaked, so stop filling
      if ( backdraw-- > 0 ) {
        MarkLeakTrail( p );
      }
      return true;
