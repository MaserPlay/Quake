...
void AddFaceToRegionSize( face_t *f )
{
  ...
  for ( i = 0; i < f->numpoints; i++ ) AddPointToRegion( f->pts[i] );
}
...
qboolean CanJoinFaces( face_t *f, face_t *f2 )
{
  ...
  if ( f2->planenum != f->planenum || f2->planeside != f->planeside ||
       f2->texturenum != f->texturenum )
    return false;
  if ( f2->outputnumber != -1 )
    return false;
  if ( f2->contents[0] !=
       f->contents[0] ) {  // does this ever happen? theyy shouldn't share.
...
void RecursiveGrowRegion( dface_t *r, face_t *f )
{
  ...
  for ( i = 0; i < f->numpoints; i++ ) {
    ...
    if ( f2 && CanJoinFaces( f, f2 ) ) {  // remove the edge and merge the faces
      edgefaces[abs( e )][0] = NULL;
      edgefaces[abs( e )][1] = NULL;
...
void RemoveColinearEdges( void )
{
  ...
  for ( i = 0; i < numedges; i++ ) edgemapping[i] = i;
...
void CountRealNumbers( void )
{
  ...
  c = 0;
  for ( i = firstmodelface; i < numfaces; i++ ) c += dfaces[i].numedges;
...
void GrowNodeRegion_r( node_t *node )
{
  ...
  for ( f = node->faces; f; f = f->next ) {
    ...
    for ( i = 0; i < MAXLIGHTMAPS; i++ ) r->styles[i] = 255;
    r->lightofs = -1;
