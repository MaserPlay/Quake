...
vec_t CastRay( vec3_t p1, vec3_t p2 )
{
  ...
  t = 0;
  for ( i = 0; i < 3; i++ ) t += ( p2[i] - p1[i] ) * ( p2[i] - p1[i] );
...
void CalcFaceVectors( lightinfo_t *l )
{
  ...
  for ( i = 0; i < 2; i++ )
    for ( j = 0; j < 3; j++ ) l->worldtotex[i][j] = tex->vecs[i][j];

  // calculate a normal to the texture axis.  points can be moved along this
  // without changing their S/T
  texnormal[0] =
      tex->vecs[1][1] * tex->vecs[0][2] - tex->vecs[1][2] * tex->vecs[0][1];
  texnormal[1] =
      tex->vecs[1][2] * tex->vecs[0][0] - tex->vecs[1][0] * tex->vecs[0][2];
  texnormal[2] =
      tex->vecs[1][0] * tex->vecs[0][1] - tex->vecs[1][1] * tex->vecs[0][0];
...
void CalcFaceExtents( lightinfo_t *l )
{
  ...
  for ( i = 0; i < s->numedges; i++ ) {
    ...
    for ( j = 0; j < 2; j++ ) {
      val = v->point[0] * tex->vecs[j][0] + v->point[1] * tex->vecs[j][1] +
...
void CalcPoints( lightinfo_t *l )
{
  ...
  for ( j = 0; j < 3; j++ )
    facemid[j] =
        l->texorg[j] + l->textoworld[0][j] * mids + l->textoworld[1][j] * midt;
...
void SingleLightFace( entity_t *light, lightinfo_t *l )
{
  ...
  if ( light->world ) {
    minlights[l->surfnum] = light->light;
    return;
  }
  ...
  if ( mapnum == l->numlightstyles ) {  // init a new light map
    ...
    for ( i = 0; i < size; i++ ) lightsamp[i] = 0;
  }
...
void FixMinlight( lightinfo_t *l )
{
  ...
  if ( i == l->numlightstyles ) {
    if ( l->numlightstyles == MAXLIGHTMAPS )
      return;  // oh well..
    for ( j = 0; j < l->numsurfpt; j++ ) l->lightmaps[i][j] = minlight;
    l->lightstyles[i] = 0;
    l->numlightstyles++;
  } else {
...
void LightFace( int surfnum )
{
  ...
  //
  f->lightofs = -1;
  for ( j = 0; j < MAXLIGHTMAPS; j++ ) f->styles[j] = 255;
  ...
  for ( i = 0; i < MAXLIGHTMAPS; i++ ) l.lightstyles[i] = 255;
  ...
  // save out the values
  //
  for ( i = 0; i < MAXLIGHTMAPS; i++ ) f->styles[i] = l.lightstyles[i];
